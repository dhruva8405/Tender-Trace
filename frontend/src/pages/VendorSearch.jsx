...
function VigilanceModal({ vendor, onClose }) {
    ...
    const refNo = `NYN-REF-${vendor?.vendor_id}-${Date.now().toString().slice(-6)}`;
    ...
    const printLetter = () => {
        ...
        win.document.write(`
<!DOCTYPE html>
<html><head><title>CBI Referral Letter — ${vendor?.vendor_id}</title>
<style>
  body{font-family:Georgia,serif;margin:60px;color:#111;line-height:1.8;font-size:14px;}
  ...
  .section h3{font-size:13px;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;border-bottom:1px solid #ccc;padding-bottom:4px;}
  table{width:100%;border-collapse:collapse;}
  td{padding:6px 10px;border:1px solid #ccc;font-size:13px;vertical-align:top;}
  td:first-child{font-weight:bold;background:#f7f7f7;width:200px;}
  .flags{color:#c00;font-weight:bold;}
...
<div class="header">
  ...
  <p><strong>Ministry of Finance — Department of Expenditure</strong></p>
  <p>Central Vigilance Commission / Tender Trace Fraud Detection System</p>
</div>
<div class="ref">
...
<div class="section">
  <p>To,<br/>The Director,<br/>Central Bureau of Investigation (CBI),<br/>Anti-Corruption Branch, New Delhi — 110001</p>
  <p><strong>Sub: Referral of vendor <span class="flags">${vendor?.vendor_name} (${vendor?.vendor_id})</span> for investigation of suspected procurement fraud — Reg.</strong></p>
  <p>Sir/Madam,</p>
  <p>This is to bring to your kind attention that the Automated Procurement Fraud Detection System (Tender Trace), deployed under the Ministry of Finance, has flagged the above-mentioned vendor for suspected coordination of procurement fraud in government contracts. The system has assigned a <strong>Risk Score of ${vendor?.score}/100 (${vendor?.risk_level})</strong> based on deterministic rule traversals across MCA21, MyNeta, and GeM datasets.</p>
</div>
<div class="section">
  <h3>Vendor Details</h3>
  <table>
    <tr><td>Vendor Name</td><td>${vendor?.vendor_name}</td></tr>
    <tr><td>Vendor ID</td><td>${vendor?.vendor_id}</td></tr>
    <tr><td>Risk Score</td><td class="flags">${vendor?.score}/100 (${vendor?.risk_level})</td></tr>
    <tr><td>Registered Address</td><td>${vendor?.address || 'As per MCA21 registry'}</td></tr>
    <tr><td>State</td><td>${vendor?.state}</td></tr>
    <tr><td>Ministry</td><td>${vendor?.ministry}</td></tr>
    <tr><td>Registration Date</td><td>${vendor?.registration_date}</td></tr>
    <tr><td>Paid-Up Capital</td><td>₹${Number(vendor?.paid_up_capital || 0).toLocaleString('en-IN')}</td></tr>
    <tr><td>Total Contract Value</td><td>₹${Number(vendor?.contract_total || 0).toLocaleString('en-IN')}</td></tr>
  </table>
</div>
<div class="section">
  <h3>Red Flags Triggered</h3>
  <table>
    ${(vendor?.flags || []).map((f, i) => `<tr><td>${i + 1}. ${f.replace('FLAG_', '').replace(/_/g, ' ')}</td><td class="flags">${f}</td></tr>`).join('')}
  </table>
</div>
<div class="section">
  <h3>Recommended Actions</h3>
  <p>1. Institute a detailed financial inquiry into all GeM contracts awarded to ${vendor?.vendor_name} (${vendor?.vendor_id}).<br/>
  2. Cross-examine director DIN linkages with other entities at the same registered address.<br/>
  3. Scrutinize ECI affidavit declarations of all associated political persons.<br/>
  4. Consider freezing disbursement of pending contract payments pending investigation.<br/>
  5. Coordinate with concerned ministry's Chief Vigilance Officer for concurrent departmental proceedings.</p>
</div>
<div class="sig">
  <p>Yours faithfully,</p>
  <br/><br/>
  <p><strong>Director, Procurement Fraud Analytics<br/>Tender Trace — AI ASCEND 2026</strong></p>
</div>
<div class="footer">Generated by Tender Trace Fraud Detection System · Powered by Amazon Bedrock & AWS Lambda · Reference: ${refNo}</div>
                ...
                {!submitted ? (
                    <>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', marginBottom: '1.5rem' }}>
                            {[
                                ...
                                { field: 'Reference No', value: refNo },
                            ].map(({ field, value }) => (
                        ...
                        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                            ...
                            <button onClick={printLetter} style={{
                                ...
                                border: 'none', borderRadius: 10, padding: '0.8rem 1.25rem',
                                color: 'white', fontWeight: 700, fontSize: '0.875rem',
                    ...
                    <div style={{ textAlign: 'center', padding: '1rem 0' }}>
                        ...
                        <div style={{ fontSize: '0.82rem', color: 'var(--text-3)', lineHeight: 1.6, marginBottom: '1.5rem' }}>
                            <span style={{ fontFamily: 'var(--mono)', color: '#34d399' }}>{refNo}</span> has been forwarded to the
